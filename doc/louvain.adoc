...
{write:true, writeProperty:'cluster'}) 
...
{weightProperty:'weight', defaultValue:1.0, write: true, writeProperty:'cluster', concurrency:4}) 
YIELD nodes, communityCount, iterations, loadMillis, computeMillis, writeMillis
...
| writeProperty | string | 'cluster' | yes | property name written back the id of the community particular node belongs to
