...
Heuristic function is for geospatial distances.
...
You will need geospatial locations
Apoc spatial comes to mind https://neo4j-contrib.github.io/neo4j-apoc-procedures/#_spatial_2
// end::constraint[]
...
[source,cypher]
----
CALL algo.shortestPath.astar.stream((startNode:Node, endNode:Node, propertyKeyLat:String, propertyKeyLon:String,
...
| propertyKeyLat | string | null | no | property name that contains weight
| propertyKeyLon | string | null | no | property name that contains weight
...
We support the following versions of the A* algorithm:
...
- implementation of Heuristic function is for geospatial distances.
