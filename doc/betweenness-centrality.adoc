...
* random: nodes are selected at random with an uniform probability of selection. Default probability is `log10(N) / e^2`. If probability is 1 algorihm works as original Brandes, where all nodes are loaded, given probability any node will load is 100%.
...
{strategy:'random', probability:1.0}) 
...
{strategy:'random', probability:1.0, writeProperty:'sampled'}) 
...
Cypher loading for now does not allow loading a single relationship as undirected but we can project it undirected if we want to. Below are the supported label-loading versions

== Versions 

Label loading supports the following versions of the betweenness centrality algorithm:
