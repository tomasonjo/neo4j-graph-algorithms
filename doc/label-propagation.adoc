...
In many real-world applications in data mining, information retrieval and pattern recognition, labeled data are usually very insufficient and labeling a huge number of data points needs expensive human labor and takes much time.
On the other hand, unlabeled data may be abundant and can be easily and cheaply obtained.
Thus how to use the labeled and unlabeled data to improve the performance becomes an important problem.
...
.Running algorithms with pre-existing labels
[source,cypher]
----
CALL algo.labelPropagation('User', 'FOLLOW','OUTGOING', {iterations:10,partitionProperty:'label', write:true}) 
