...
We can describe the subset of your graph you want to project using label for nodes and relationship-type for relationships.
...
If label and relationship-type are not selective enough to describe our subgraph to run the algorithm on, we can use Cypher statements to project subsets of your graph.
...
We can also return a property value or weight (according to your config) in addition to the id's from these statements.
...
With cypher projection we can also project a virtual (non-stored) graph. 
Example how to project a stored bi-partite graph to a virtual projected mono-partite graph. 
